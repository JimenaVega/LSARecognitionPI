...
def create_pose_tracker():
	...
	pose_tracker = mp_pose.Pose(static_image_mode=False) # Se reduce a 25 landmarks

	pose_draw = mp.solutions.drawing_utils

	return pose_tracker, pose_draw

face_landmarker = create_face_landmarker()
pose_tracker, pose_draw = create_pose_tracker()
...
while video.isOpened():
	...
	if frame_exists:
		...
		if frame_no % 3 == 0:
			...
			pose_tracker_result = pose_tracker.process(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))

			annotated_image = draw_landmarks_on_image(frame, face_landmarker_result)
			pose_draw.draw_landmarks(annotated_image, pose_tracker_result.pose_landmarks, mp.solutions.pose.POSE_CONNECTIONS)
