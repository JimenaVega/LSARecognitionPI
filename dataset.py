    ...
    with mp.solutions.holistic.Holistic(min_detection_confidence=0.75, min_tracking_confidence=0.75) as holistic:
        ...
        for file_name in files:
            ...
            for i in range(clips_frames):
                ...
                shoulders_distance = distance.euclidean((left_shoulder.x, left_shoulder.y, left_shoulder.z), (right_shoulder.x, right_shoulder.y, right_shoulder.z))
                ...
                lh = np.array([[(res.x - nose.x) / shoulders_distance, (res.y - nose.y) / shoulders_distance]
                               for res in results.left_hand_landmarks.landmark]).flatten() if results.left_hand_landmarks else np.zeros(HAND_FLATTEN_POINTS)
                ...
                rh = np.array([[(res.x - nose.x) / shoulders_distance, (res.y - nose.y) / shoulders_distance]
                               for res in results.right_hand_landmarks.landmark]).flatten() if results.right_hand_landmarks else np.zeros(HAND_FLATTEN_POINTS)
                ...
                pose_landmarks = np.array([[(res.x - nose.x) / shoulders_distance, (res.y - nose.y) / shoulders_distance]
                                           for res in results.pose_landmarks.landmark[0:23]]).flatten() if results.pose_landmarks else np.zeros(POSE_FLATTEN_POINTS)
